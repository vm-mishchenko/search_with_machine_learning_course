...
index_body = {
    ...
    "analysis": {
      ...
      "analyzer": {
        "body_pos": {
          ...
          # we can't use standard b/c it strips or delimiters
  ...
  "mappings": {
    "properties": {
      ...
      "body_pos": {"type": "text", "analyzer": "body_pos",
...
for doc in docs:
  ...
  for item in ["body"]:  # Just do body for now
    ...
    named_entities = nltk.ne_chunk(part_of_speach)
    doc["%s_sentences" % item] = " ".join(
        "__SB__ %s __SE__" % x for x in sentences)
    doc["%s_pos" % item] = " ".join(
        ["#".join([x[0], "__%s__" % x[1]]) for x in part_of_speach])
    doc["%s_ne" % item] = get_entities(named_entities,
