...
# responds to an Elasticsearch index used to store metadata about the features
# and models. Typically, one feature store corresponds to a major search
# site/implementation. For example, wikipedia vs wikitravel
...
featureset_path = urljoin(ltr_model_path + "/",
                          "_featureset/{}".format(featureset_name))
...
resp = requests.post(featureset_path, headers=headers,
                     data=json.dumps(ltr_feature_set), auth=auth, verify=False)
...
class Judgment:
  ...
  def __init__(self, query, doc_id, display_name, grade=0, features=[],
      query_str=None):
    self.query = query
  ...
  def toXGBFormat(self):
    featuresAsStrs = ["%s:%s" % (idx + 1, feature.get('value', 0)) for
                      idx, feature in enumerate(self.features)]
    comment = "# %s\t%s" % (self.doc_id, self.query_str)
    return "%s\tqid:%s\t%s %s" % (
      self.grade, self.query, "\t".join(featuresAsStrs), comment)
    ...
    for hit in hits:
      ...
      for input in sys.stdin.readline():
        ...
        if grade == "0" or grade == "1":
          judgment = Judgment(query, hit['_id'], hit['_source']['title'],
                              int(grade))
...
for (idx, item) in enumerate(judgments.items()):
  ...
  for judgment in judge_vals:
    ...
    if response and len(response['hits']) > 0 and len(
...
response = requests.post(model_path, data=json.dumps(os_model), headers=headers,
  ...
  "query": {
    "rescore_query": {
      "sltr": {
        ...
        "store": ltr_store_name,
        "active_features": [title_query_feature_name, body_query_feature_name,
